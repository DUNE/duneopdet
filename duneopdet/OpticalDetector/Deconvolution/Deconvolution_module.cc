...
namespace opdet
{
        ...
        fxG1(fSamples)
  {
    ...
    if (fApplyPostfilter)
      BuildExtraFilter(fxG1, fPostfilterConfig);
  }
  ...
  void Deconvolution::produce(art::Event &evt)
  {
    ...
    for (auto const &wf : digi_wave)
    {
      ...
      else if (strcmp(fOutputProduct.c_str(), "v") == 0)
      {
        ...
        for (int i = 0; i < fSamples; i++)
          out_recob_float.at(i) = scale * vv[i];
      }
    ...
    if (fFilterConfig.fType == Deconvolution::kWiener)
    {
      printf("***Wiener Filter****");
    }
    if (fFilterConfig.fType == Deconvolution::kGauss)
    {
      printf("***Gauss Filter***");
    }
    if (fFilterConfig.fType == Deconvolution::kOther)
    {
      printf("***Standart dec***");
    }
    if (fApplyPostfilter)
    {
      printf("***ApplyPostfilter***");
    }
  ...
  Double_t Deconvolution::ComputeAutoNormalization(CmplxWaveform_t &xGH, const float thrs)
  {
    ...
    while (x[ileft] > thrs && ileft > 0)
      ileft--;
    while (x[iright] > thrs && iright < fSamples)
      iright++;

    double norm = 0;
    for (Int_t k = ileft; k <= iright; k++)
      norm += x[k];
