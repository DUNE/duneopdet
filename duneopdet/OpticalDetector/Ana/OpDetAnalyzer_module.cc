...
void opdet::OpDetAnalyzer::analyze(art::Event const& evt)
{
  ...
  // reset vectors
  fNOpDetAll.clear(); fNOpDetAll.resize(art::ServiceHandle<geo::WireReadout>()->Get().NOpChannels(), 0);
  fNOpDetDirect.clear(); fNOpDetDirect.resize(art::ServiceHandle<geo::WireReadout>()->Get().NOpChannels(), 0);
  fNOpDetReflected.clear(); fNOpDetReflected.resize(art::ServiceHandle<geo::WireReadout>()->Get().NOpChannels(), 0);
  ...
  for (unsigned int OpDet = 0; OpDet < art::ServiceHandle<geo::WireReadout>()->Get().NOpChannels(); OpDet++) {
