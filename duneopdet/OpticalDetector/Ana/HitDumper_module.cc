...
void Hitdumper::analyze(const art::Event& evt)
{
  ...
  if (freadOpHits) {
    ...
    for (auto ophit_label : fOpHitsModuleLabels) {
      ...
      for (size_t i = 0; i < ophitlist.size(); ++i) {
        ...
        _ophit_opdet[index] = art::ServiceHandle<geo::WireReadout const>()->Get().OpDetFromOpChannel(ophitlist.at(i)->OpChannel());
        ...
        auto opdet_center = art::ServiceHandle<geo::WireReadout const>()->Get().OpDetGeoFromOpChannel(ophitlist.at(i)->OpChannel()).GetCenter();
  ...
  if (freadTruth){
    ...
    art::FindMany<simb::MCFlux> fmFluxNeutrino(mctruthListHandle, evt, fGenieGenModuleLabel);
    // Get GTruth information for scattering code
    art::FindMany< simb::GTruth > fmgt( mctruthListHandle, evt, fGenieGenModuleLabel );
    ...
    if (mcevts_truth > 0){//at least one mc record
      ...
      for (unsigned int i_mctruth = 0; i_mctruth < mclist.size(); i_mctruth++){
       ...
       // if (fmgt.size()>i_mctruth) {
        ...
        // temporary comment of two lines above to make it compile, Andrzej
