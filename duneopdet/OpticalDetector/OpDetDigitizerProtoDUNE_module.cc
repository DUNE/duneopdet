...
//
// Module specific for ProtoDUNE-SP to be able to use different QE for different PDs and other differences, if needed
// Created: 18/Jun/2021 - L. Paulucci
// Based on OpDetDigitizerDUNE_module.cc
// Modified: 
//=========================================================

#ifndef OpDetDigitizerProtoDUNE_h
#define OpDetDigitizerProtoDUNE_h
...
namespace opdet {
  ...
  public:
      ...
      //      virtual ~OpDetDigitizerProtoDUNE();
    ...
    private:
      ...
      double  fQEOverride; //bars
   //   double  fRefQEOverride; //Removed since there is no reflected light in ProtoDUNE-SP
...
namespace opdet {
  ...
  DEFINE_ART_MODULE(OpDetDigitizerProtoDUNE)
...
namespace opdet {
  ...
  //---------------------------------------------------------------------------
  // Constructor
  OpDetDigitizerProtoDUNE::OpDetDigitizerProtoDUNE(fhicl::ParameterSet const& pset)
    : EDProducer{pset}
...
//           = 
    ...
    double tempQEArapucaBeam    = pset.get< double >("QEArapucaBeamOverride", 0     );
    double tempQEArapucaNonBeam    = pset.get< double >("QEArapucaNonBeamOverride", 0     );

    fQEOverride         = 0;
    fQEArapucaBeamOverride      = 0;
    fQEArapucaNonBeamOverride      = 0;
    
    if (tempQE > 0) {
      mf::LogInfo("OpDetDigitizerProtoDUNE") << "Overriding QE. All the functions of OpDetResponseService being ignored.";
      ...
      if (fQEOverride > 1.0001) {
        mf::LogError("OpDetDigitizerProtoDUNE") << "Quantum efficiency set as OpDetDigitizerProtoDUNE.QEOverride, " << tempQE
    ...
    if (tempQEArapucaBeam > 0) {
      mf::LogInfo("OpDetDigitizerProtoDUNE") << "Overriding QE. All the functions of OpDetResponseService being ignored.";
        
      // Correct out the prescaling applied during simulation
      auto const *LarProp = lar::providerFrom<detinfo::LArPropertiesService>();
      fQEArapucaBeamOverride = tempQEArapucaBeam / LarProp->ScintPreScale();
      
      if (fQEArapucaBeamOverride > 1.0001) {
        mf::LogError("OpDetDigitizerProtoDUNE") << "Quantum efficiency set as OpDetDigitizerProtoDUNE.QEOverride, " << tempQEArapucaBeam
                                           << " is too large.  It is larger than the prescaling applied during simulation, "
                                           << LarProp->ScintPreScale()
    ...
    if (tempQEArapucaNonBeam > 0) {
      mf::LogInfo("OpDetDigitizerProtoDUNE") << "Overriding QE. All the functions of OpDetResponseService being ignored.";
        
      // Correct out the prescaling applied during simulation
      auto const *LarProp = lar::providerFrom<detinfo::LArPropertiesService>();
      fQEArapucaNonBeamOverride = tempQEArapucaNonBeam / LarProp->ScintPreScale();
      
      if (fQEArapucaNonBeamOverride > 1.0001) {
        mf::LogError("OpDetDigitizerProtoDUNE") << "Quantum efficiency set as OpDetDigitizerProtoDUNE.QEOverride, " << tempQEArapucaNonBeam
                                           << " is too large.  It is larger than the prescaling applied during simulation, "
                                           << LarProp->ScintPreScale()
  ...
  //---------------------------------------------------------------------------
  void OpDetDigitizerProtoDUNE::produce(art::Event& evt)
  {
  ...
  //---------------------------------------------------------------------------
  void OpDetDigitizerProtoDUNE::AddPulse(size_t timeBin,
  ...
  //---------------------------------------------------------------------------
  double OpDetDigitizerProtoDUNE::Pulse1PE(double time) const
  {
  ...
  //---------------------------------------------------------------------------
  void OpDetDigitizerProtoDUNE::CreateSinglePEWaveform()
  {
      ...
      for (size_t i = 0; i<time_sdps_vector.size(); ++i) //I is now the time bin.
      {
        ...
        for (auto const& sdp : time_sdps.second)
        {
          ...
          for(int j=0; j<sdp.numPhotons;++j)
          {
            ...
            //mf::LogInfo("OpDetDigitizerProtoDUNE")
  ...
  //---------------------------------------------------------------------------
  unsigned short OpDetDigitizerProtoDUNE::CrossTalk() const
  {
  ...
  //---------------------------------------------------------------------------
  std::vector< short > OpDetDigitizerProtoDUNE::VectorOfDoublesToVectorOfShorts
  ...
  std::map< size_t, std::vector< short > > OpDetDigitizerProtoDUNE::
        ...
        const FocusList& fls)
    {
      ...
      raw::OpDetWaveform waveform(-1, -99999.,std::vector<short unsigned int> (waveform_short.begin(),  waveform_short.end())  );
  ...
  //---------------------------------------------------------------------------
  double OpDetDigitizerProtoDUNE::GetDriftWindow(detinfo::DetectorPropertiesData const& detProp) const
  {
  ...
  //---------------------------------------------------------------------------
  double OpDetDigitizerProtoDUNE::TickToTime(size_t tick) const
  {
  ...
  //---------------------------------------------------------------------------
  size_t OpDetDigitizerProtoDUNE::TimeToTick(double time) const
  {
  ...
  //---------------------------------------------------------------------------
  void OpDetDigitizerProtoDUNE::CheckFHiCLParameters() const
  {
  ...
  //---------------------------------------------------------------------------
  bool OpDetDigitizerProtoDUNE::Detected(int OpDet, int &readoutChannel, bool Reflected) {

    if ( (!Reflected && fQEOverride > 0) ) {
      ...
      // Check QE
      if (OpDet>28 && OpDet<45) { //Arapucas on APA 6
        if ( CLHEP::RandFlat::shoot(1.0) > fQEArapucaNonBeamOverride ) return false;
      }
      else {
        if (OpDet>44 && OpDet<61) { //Arapucas on APA 3
          if ( CLHEP::RandFlat::shoot(1.0) > fQEArapucaBeamOverride ) return false;        
        }
        else{ //bars
